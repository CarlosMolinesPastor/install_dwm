...
downloadDependencies() {
    if grep "Arch\|Artix\|EndeavourOS\|Manjaro" /etc/*-release; then
        ...
        if [[ -e /usr/bin/paru ]]; then
            ...
            viewnior mpv htop lxappearance picom rofi rsync pavucontrol dunst                 \
            ranger python-pip noto-fonts-emoji noto-fonts-cjk xwallpaper xcolor imlib2 neovim  \
            exa bat file-roller gvfs gvfs-mtp xclip firefox libxft-bgra rofi-emoji fzf        \
            xorg-xsetroot ytfzf cava xdg-user-dirs noto-fonts xdotool scrot ueberzug         \
